...
namespace tacticode
{
	namespace spell
	{
		...
		{
		public:
			Spell(std::list<std::shared_ptr<IEffect> > effect, std::string const& name, float power, size_t range, size_t cooldown, size_t active, size_t aoe, size_t los, size_t minRange = 0, size_t nbTurn = 1);
